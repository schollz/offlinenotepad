...
<p>More than one device can open the same notebook. Revisions keep changes in order. If a newer server revision arrives while this device has an unsent edit, the local edit is rebased and retried in place, so synchronization never creates duplicate conflict notes.</p>
...
<p>Sync was rebuilt around encrypted records, optimistic revisions, and permanent tombstones. Those details mostly stay out of the way. Pending edits are rebased onto newer revisions and retried in place, so the latest successfully synchronized version remains the note without generating duplicate conflict copies.</p>
